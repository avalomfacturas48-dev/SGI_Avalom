...
export async function GET(
  ...
  return authenticate(async (req: NextRequest, res: NextResponse) => {
    try {
      ...
      const payment = await prisma.ava_pago.findUnique({
...
export async function PUT(
  ...
  return authenticate(async (req: NextRequest, res: NextResponse) => {
    try {
      ...
      const updatedPayment = await prisma.ava_pago.update({
        where: { pag_id: BigInt(pagId) },
        data: {
          pag_monto: BigInt(data.pag_monto),
      ...
      return NextResponse.json(
        { success: true, data: stringifyWithBigInt(updatedPayment) },
        { status: 200 }
      );
    } catch (error) {
